...
function initCommand() {
  const targetPath = (0, import_node_path2.join)(process.cwd(), ".agent-teams");
  ...
  (0, import_node_fs2.mkdirSync)((0, import_node_path2.join)(targetPath, "artifacts"), { recursive: true });
  (0, import_node_fs2.mkdirSync)((0, import_node_path2.join)(targetPath, "artifacts", "implementation"), { recursive: true });
  (0, import_node_fs2.mkdirSync)((0, import_node_path2.join)(targetPath, "artifacts", "review-reports"), { recursive: true });
  (0, import_node_fs2.mkdirSync)((0, import_node_path2.join)(targetPath, "worktrees"), { recursive: true });
  const opencodeSrc = (0, import_node_path2.join)(findDefaultsPath(), ".opencode");
  if ((0, import_node_fs2.existsSync)(opencodeSrc)) {
    const opencodeDest = (0, import_node_path2.join)(process.cwd(), ".opencode");
...
Created: .agent-teams/
  \u251C\u2500\u2500 pipeline.yaml
  \u251C\u2500\u2500 agents/        (${countFiles((0, import_node_path2.join)(targetPath, "agents"))} agents)
  \u251C\u2500\u2500 skills/        (${countFiles((0, import_node_path2.join)(targetPath, "skills"))} skills)
  \u251C\u2500\u2500 standards/     (${countFilesRecursive((0, import_node_path2.join)(targetPath, "standards"))} standards)
  \u251C\u2500\u2500 artifacts/
  \u2514\u2500\u2500 worktrees/
...
function findDefaultsPath() {
  const candidates = [
    (0, import_node_path2.join)(__dirname, "..", "..", "defaults"),
    (0, import_node_path2.join)(__dirname, "..", "defaults")
  ];
...
function copyDir(src, dest) {
  ...
  for (const entry of (0, import_node_fs2.readdirSync)(src)) {
    const srcPath = (0, import_node_path2.join)(src, entry);
    const destPath = (0, import_node_path2.join)(dest, entry);
    if ((0, import_node_fs2.statSync)(srcPath).isDirectory()) {
      copyDir(srcPath, destPath);
    } else {
      (0, import_node_fs2.cpSync)(srcPath, destPath);
    }
  }
}
function countFiles(dir) {
  return (0, import_node_fs2.readdirSync)(dir).filter((f) => (0, import_node_fs2.statSync)((0, import_node_path2.join)(dir, f)).isFile()).length;
}
...
function loadPipelineConfig(pipelinePath) {
  const content = (0, import_node_fs3.readFileSync)(pipelinePath, "utf-8");
...
var STATE_FILE = ".agent-teams/state.json";
...
function loadState() {
  if (!(0, import_node_fs4.existsSync)(STATE_FILE)) {
    throw new Error('No pipeline running. Run "agent-teams start" first.');
  }
  const content = (0, import_node_fs4.readFileSync)(STATE_FILE, "utf-8");
  return JSON.parse(content);
}
function saveState(state) {
  (0, import_node_fs4.writeFileSync)(STATE_FILE, JSON.stringify(state, null, 2), "utf-8");
}
...
function initPipeline(pipelinePath) {
  if ((0, import_node_fs5.existsSync)(".agent-teams/state.json")) {
...
function startStage(state, stageName) {
  ...
  stage.started_at = (/* @__PURE__ */ new Date()).toISOString();
  stage.attempt++;
  saveState(state);
  return state;
}
...
function markPassed(state, stageName) {
  const stage = state.stages[stageName];
  stage.status = "passed";
  stage.completed_at = (/* @__PURE__ */ new Date()).toISOString();
...
function startCommand(requirement, options) {
  ...
  if (!(0, import_node_fs6.existsSync)(pipelinePath)) {
    console.error('No .agent-teams/pipeline.yaml found. Run "agent-teams init" first.');
    process.exit(1);
  }
  const artifactsDir = ".agent-teams/artifacts";
  if (!options.clean && (0, import_node_fs6.existsSync)(artifactsDir)) {
    const files = (0, import_node_fs6.readdirSync)(artifactsDir);
    if (files.length > 0) {
  ...
  if (options.clean) {
    if ((0, import_node_fs6.existsSync)(".agent-teams/state.json")) {
      (0, import_node_fs6.rmSync)(".agent-teams/state.json");
    }
    const dirs = ["artifacts", "worktrees"];
    for (const dir of dirs) {
      const full = (0, import_node_path3.join)(".agent-teams", dir);
      if ((0, import_node_fs6.existsSync)(full)) {
        for (const entry of (0, import_node_fs6.readdirSync)(full)) {
          (0, import_node_fs6.rmSync)((0, import_node_path3.join)(full, entry), { recursive: true, force: true });
        }
      }
    }
  ...
  if (options.spec) {
    if (!(0, import_node_fs6.existsSync)(options.spec)) {
      console.error(`Spec file not found: ${options.spec}`);
      process.exit(1);
    }
    finalRequirement = (0, import_node_fs6.readFileSync)(options.spec, "utf-8");
...
function parseFrontmatter(filePath) {
  const content = (0, import_node_fs7.readFileSync)(filePath, "utf-8").replace(/\r\n/g, "\n");
...
function runGit(args) {
  try {
    const output = (0, import_node_child_process2.execSync)(`git ${args}`, { encoding: "utf-8", stdio: "pipe" });
...
function isGitRepo() {
  return (0, import_node_fs8.existsSync)(".git");
}
function cleanWorktrees(baseDir) {
  if ((0, import_node_fs8.existsSync)(baseDir)) {
    (0, import_node_fs8.rmSync)(baseDir, { recursive: true, force: true });
  }
}
function mergeWorktree(batchId, baseDir) {
  const worktreePath = (0, import_node_path4.join)(baseDir, batchId);
...
// src/commands/next.ts
var BASE2 = ".agent-teams";
function resolvePath(relativePath) {
  return (0, import_node_path5.join)(BASE2, relativePath);
}
function nextCommand(requirement) {
  ...
  if (stageState.status === "pending") {
    ...
    if (!(0, import_node_fs9.existsSync)(artifactPath)) {
    ...
    if (!(0, import_node_fs9.existsSync)(reviewOutput)) {
    ...
    stageState.review_verdict = verdict ?? void 0;
    setHumanReviewStage(state, state.current_stage);
...
function cleanStageArtifacts(stageName) {
  ...
  if ((0, import_node_fs10.existsSync)(artifactPath)) (0, import_node_fs10.rmSync)(artifactPath);
  const reviewPath = `.agent-teams/artifacts/review-reports/${stageName}-review.md`;
  if ((0, import_node_fs10.existsSync)(reviewPath)) (0, import_node_fs10.rmSync)(reviewPath);
}
...
function artifactCommand(stageName) {
  ...
  if (!artifactPath || !(0, import_node_fs11.existsSync)(artifactPath)) {
    console.error(`No artifact found for stage "${stageName}"`);
    return;
  }
  const fm = parseFrontmatter(artifactPath);
  const content = (0, import_node_fs11.readFileSync)(artifactPath, "utf-8");
  console.log(`
...
function listCommand(resource) {
  const basePath = (0, import_node_path6.join)(".agent-teams");
  const resourcePath = (0, import_node_path6.join)(basePath, resource);
  if (!(0, import_node_fs12.existsSync)(resourcePath)) {
...
${resource.toUpperCase()}:`);
  ...
  if (resource === "standards") {
    ...
    for (const file of (0, import_node_fs12.readdirSync)(resourcePath)) {
...
function listRecursive(dir, prefix) {
  for (const entry of (0, import_node_fs12.readdirSync)(dir)) {
    ...
    if ((0, import_node_fs12.statSync)(full).isDirectory()) {
...
program2.name("agent-teams").description("AI Agent Teams for end-to-end software delivery").version("0.1.0");
program2.command("init").description("Initialize Agent Teams in the current workspace").option("--clean", "Overwrite existing .agent-teams/").action((options) => {
  if (options.clean && (0, import_node_fs13.existsSync)(".agent-teams")) {
    (0, import_node_fs13.rmSync)(".agent-teams", { recursive: true, force: true });
  }
  initCommand();
});
program2.command("start [requirement]").description("Start a new pipeline").option("--spec <path>", "Load requirements from file").option("--clean", "Clear previous state").action((requirement, options) => {
  startCommand(requirement ?? "", options);
});
program2.command("status").description("Show pipeline status").action(() => statusCommand());
program2.command("next").description("Advance to the next stage").action(() => nextCommand());
program2.command("approve <stage>").description("Approve a stage after human review").action((stage) => approveCommand(stage));
program2.command("reject <stage>").description("Reject a stage and retry").option("--reason <reason>", "Reason for rejection").action((stage, options) => rejectCommand(stage, options.reason));
program2.command("retry <stage>").description("Retry a failed stage").action((stage) => retryCommand(stage));
program2.command("artifact <stage>").description("Show artifact for a stage").action((stage) => artifactCommand(stage));
program2.command("report").description("Show pipeline summary").action(() => reportCommand());
program2.command("list <resource>").description("List agents, skills, or standards").action((resource) => {
